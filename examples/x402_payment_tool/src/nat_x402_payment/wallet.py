...
class InlineWalletSigner(WalletSigner):
    ...
    def __init__(self, private_key: str | None = None):
        ...
        if not self._private_key:
            raise ValueError("WALLET_PRIVATE_KEY not set. For production, use WALLET_SIGNER_URL instead.")
...
def create_wallet_signer(signer_url: str | None = None) -> WalletSigner:
    """
    Factory function that creates the appropriate signer based on config/environment.

    Priority:
    1. Explicit signer_url param or WALLET_SIGNER_URL env → RemoteWalletSigner (production)
    2. WALLET_PRIVATE_KEY env → InlineWalletSigner (development)
    """
    signer_url = signer_url or os.getenv("WALLET_SIGNER_URL", "")
    if signer_url:
    ...
    raise ValueError("No wallet signer configured. Set WALLET_SIGNER_URL (production) "
